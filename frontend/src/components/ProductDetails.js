...
const ProductDetails = () => {
  ...
  const handleAddToCart = async () => {
    ...
    try {
      const response = await axios.put(`${BACKEND_URL_LINK}/api/v1/addItem`, {
        userId: userId,
        productId: id,
      });

      if (response.status === 200) {
        toast.success(response.data.message || "Item successfully added to cart!");
      }
    } catch (error) {
    ...
    <div className="flex flex-col lg:flex-row bg-gray-100 p-6 w-full max-w-7xl mx-auto">
      ...
      <div className="lg:w-1/2 lg:pl-8 text-left">
        ...
        <div className="mt-6 flex space-x-4">
          <button onClick={handleBuy} className="px-6 py-3 bg-orange-500 text-white rounded hover:bg-orange-600">
            Buy Now
          </button>
          <button
